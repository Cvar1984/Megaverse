...
A port of [Miniverse](https://github.com/Cvar1984/Miniverse), which does the same
thing on Garmin Connect IQ. The sky maths is the same maths; what changed is the
platform underneath it, and [Differences from Miniverse](#differences-from-miniverse)
is the short version of what that moved.
...
- [Differences from Miniverse](#differences-from-miniverse)
...
64-bit by default, so the care the Garmin build had to take by hand is the default
here.

## 2. Where the object is

**Stars** are a table of J2000 right ascension, declination and visual magnitude —
28 of them, ordered by brightness. Precession and proper motion are ignored: both
are far under what a wrist compass resolves.
...
**This is where the port differs most.** The Garmin build assembles the pointing
frame itself: it reads a raw accelerometer and magnetometer, negates gravity to
recover "up", flattens the field against it to get north, crosses the two for east,
and recovers the local magnetic declination by watching the system compass while the
watch is near level.

Android ships all of that.
...
*exactly* the layout the Garmin build constructs by hand. Being gyro-fused, it
needs no smoothing of its own, so the vector low-pass filter goes too.
...
the position, east-positive, which is the convention the rotation wants. So the
declination is a model lookup rather than something inferred from a system heading
while the watch happens to be level:
...
**Constellations.** Eighteen stick figures, 145 stars, 35 strokes — all twelve of
the zodiac, which is what the ecliptic runs through, plus six bright ones off it
that most people can already find.
...
lunar path at the Moon's position at each of the next 27 midnights. Twenty-seven is
one full circuit — a fortnight only dates half the circle, and since the line is
drawn whole you then get long stretches carrying no date at all.
...
## Differences from Miniverse

| | Miniverse | Megaverse |
|---|---|---|
| Platform | Connect IQ, Monkey C | Wear OS, Kotlin + Compose |
| Attitude | accelerometer + magnetometer, assembled by hand | `TYPE_ROTATION_VECTOR`, fused |
| Smoothing | vector low-pass, hand-rolled | none needed; the fusion is stabilised |
| Declination | inferred from the system heading while near level | `GeomagneticField` model lookup |
| Julian Day | Gregorian algorithm, forced into 64-bit by hand | one line from epoch millis; `Double` is 64-bit |
| Screen wake | re-armed around burn-in protection | `View.keepScreenOn` |
| Grid ceiling | coarser on watches with under 128 KB | no ceiling |
| Menu | hardware menu button | long press, without consuming the swipe |
| Constellations | 8 figures | 18 figures, all 12 zodiac |
| Paths | line only | line, or line with dates along it |
| Calendar | — | rise, set and phase for a fortnight |

The port is not a transliteration. Roughly a third of the original's code was
answering questions Android answers itself, and deleting it was most of the work.
What is left is the astronomy, which is the same astronomy.
...
92 JVM unit tests, 97.4% line coverage over the sky maths, the catalogue and the
settings ring. The Compose screens and the canvas drawing are excluded — a unit test
cannot exercise them, and counting them would only dilute the number that says
whether the maths is tested.
